class Central
  class Deploy
    ...
    def self.install_curl(ip)
      Central.debug "Installing curl on Node #{ip}"

      command = "ssh -p 22 root@#{ip} 'yum install -y curl'"
    ...
    def self.install_epel(ip)
      Central.debug "Installing Epel on Node #{ip}"

      command = "ssh -p 22 root@#{ip} 'rpm -ivh http://dl.fedoraproject.org/pub/epel/6/i386/epel-release-6-7.noarch.rpm'"
      log = Log.new object_id
      b_stdout = Log::Buffer.new object_id, "stdout"
      b_stderr = Log::Buffer.new object_id, "stderr"
